...
class SubjectRepository:
    ...
    def get_by_id(self, id):
        subject_model = self.session.query(SubjectModel).filter(SubjectModel.id == id).first()
        return SubjectMapper.model_to_subject(subject_model) if subject_model else None
    
    def get_by_user_id(self, user_id):
        subject_models = self.session.query(SubjectModel).filter(SubjectModel.user_id == user_id).all()
        return [SubjectMapper.model_to_subject(subject_model) for subject_model in subject_models] if subject_models else None

    def get_all(self):
        subject_models = self.session.query(SubjectModel).all()
        return [SubjectMapper.model_to_subject(subject_model) for subject_model in subject_models]
