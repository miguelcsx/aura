...
class TopicRepository:
    ...
    def get_by_id(self, id):
        topic_model = self.session.query(
            TopicModel).filter(TopicModel.id == id).first()
        return TopicMapper.model_to_topic(topic_model) if topic_model else None

    def get_by_subject_id(self, subject_id):
        topic_models = self.session.query(TopicModel).filter(
            TopicModel.subject_id == subject_id).all()
        return [TopicMapper.model_to_topic(
            topic_model) for topic_model in topic_models] if topic_models else None

    def get_all(self):
        topic_models = self.session.query(TopicModel).all()
        return [TopicMapper.model_to_topic(
            topic_model) for topic_model in topic_models]
